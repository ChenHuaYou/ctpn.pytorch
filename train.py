...
from ctpn.dataset import ICDAR2015Dataset, transform
...
def my_collate(batch):
    ...
    clss = [item[1] for item in batch]
    regrs = [item[2] for item in batch]
    imgs = torch.transpose(pad_sequence(imgs),0,1)
    clss = torch.transpose(pad_sequence(clss),0,1)
    regrs = torch.transpose(pad_sequence(regrs),0,1)
    return [imgs, clss, regrs]

if __name__ == '__main__':
    dataset = ICDAR2015Dataset(config.img_dir, config.label_dir, transform)
    dataloader = DataLoader(dataset, batch_size=4, shuffle=True, num_workers=config.num_workers, collate_fn=my_collate)
    ...
    for epoch in range(resume_epoch+1, epochs):
        ...
        for batch_i, (imgs, clss, regrs) in enumerate(dataloader):
            ...
            out_clss, out_regrs = model(imgs)
            loss_regr = critetion_regr(out_regrs, regrs)
            loss_cls = critetion_cls(out_clss, clss)
